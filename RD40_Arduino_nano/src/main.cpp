...
long lastAnimationUpdateTime;       // Timestamp for animation update

bool animationMode = true;          // Flag to enable/disable animation
int animationAngle = 0;             // Current angle for animation

// I2C Buffer
unsigned char i2cBuffer[16];        // Buffer for I2C received data
int receiveEventFlag = 0;           // Flag to indicate I2C receive event

// Gear Image Data (stored in PROGMEM)
const PROGMEM uint8_t gearImage[GEAR_IMAGE_ROWS][5] = {
...
void loop() {
  ...
  if (animationMode && (millis() - lastAnimationUpdateTime) > ANIMATION_UPDATE_INTERVAL) {
    animationAngle = (animationAngle + 1) % LINE_COUNT; // Increment angle and wrap around
    drawGear(animationAngle);                          // Draw gear at new angle   
    lastAnimationUpdateTime = millis();                // Update timestamp
  }
...
void receiveEventHandling() {
  ...
  if (messageCount == 6) {
    if (i2cBuffer[0] < LINE_COUNT) {
      ...
      // Update brightness and animation mode
      brightness = i2cBuffer[1];
      animationMode = i2cBuffer[2] != 0;
    }
...
void drawGear(int theta) {
  for (int i = 0; i < LINE_COUNT; i++) {
    int rowIndex = theta % GEAR_IMAGE_ROWS; // Calculate row index in gear image
    for (int j = 0; j < 5; j++) {
      lineData[i][j] = pgm_read_byte(&(gearImage[rowIndex][j])); // Copy gear data to line buffer
    }
    theta = (theta + 1) % LINE_COUNT; // Increment angle and wrap around
  }
}